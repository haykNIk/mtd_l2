/*
 * @Author: hayknik
 */
#include "Form.h"

#include <cmath>
#include <iostream>

namespace
{
    void fill(char* str, unsigned int len, char c)
    {
        for (unsigned int i = 0; i < len; ++i)
            str[i] = c;
        str[len] = '\0';
    }
}

void Triangle::setOffset(unsigned int offset)
{
    m_offset = offset;
}

void Circle::setOffset(unsigned int offset)
{
    m_offset = offset;
}

void Circle::setRadius(unsigned int radius)
{
    m_radius = radius;
}

void Triangle::draw() const
{
    char* spaces = new char[m_offset + 1];
    fill(spaces, m_offset, ' ');

    char* top = new char[m_leg_length + 1];
    fill(top, m_leg_length, '-');

    std::cout << spaces << '|' << top << std::endl;

    for(int i=0; i < m_leg_length; i++)
    {
        std::cout << spaces;
        std::cout << '|';
        for (int j=0; j < m_leg_length - i - 1; ++j)
            std::cout << ' ';
        std::cout << '/' << std::endl;
    }
    delete[] spaces;
    delete[] top;
}

// Метод рисует окружность в консоли в виде символов '*'.
//
// Алгоритм: окружность вписывается в квадрат со стороной, равной диаметру
// (2 * m_radius). Для каждой клетки квадрата вычисляется расстояние от центра
// по формуле x^2 + y^2; если оно не превышает квадрат радиуса, клетка попадает
// внутрь окружности и печатается '*', иначе печатается пробел.
// Перед каждой строкой выводится m_offset пробелов — горизонтальный отступ,
// задаваемый через setOffset.
void Circle::draw() const
{
    char* spaces = new char[m_offset + 1];
    fill(spaces, m_offset, ' ');

    const unsigned int diameter = 2 * m_radius;
    const unsigned int radiusSquared = m_radius * m_radius;

    for(int iRow = 0; iRow < diameter; ++iRow)
    {
        std::cout << spaces;
        for(int iCol = 0; iCol < diameter; ++iCol)
        {
            int x = iCol - m_radius;
            int y = iRow - m_radius;
            bool isInside = (x*x + y*y) <= radiusSquared;
            std::cout << (isInside ? '*' : ' ');
        }
        std::cout << std::endl;
    }
    delete[] spaces;
}
